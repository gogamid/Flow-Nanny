...
control MyIngress(inout headers hdr,
    ...
    register<bit<1>>(3) isSeenPortEgress; //in order to identify first packet in the port(0 first Packet, 1 not)

    bit<48> _startTimePortEgress;
    register<bit<48>>(3) startTimePortEgress; //start time of port with index of port

    bit<32> link_load_egress;
    register<bit<32>>(3) linkLoadEgress; //current port based counter

    bit<32> prevPortBasedBytesEgress; //local help variable which is used with register "bytesReceivedPort"
    register<bit<32>>(3) bytesSentPort; //number of bytes received per port
