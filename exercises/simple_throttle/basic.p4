...
const bit<32> portBasedByteLimit=1000;    //in the given window
const bit<48> link_level_window=1000000; //frequency that we reset the bytes received, in microseconds(1s)  linkload for this windows separrate
const bit<48> flow_level_window=10*link_level_window; //frequency that we reset the bytes received, in microseconds(10s)
    ...
    */
    ...
    apply {
        ...
        if(standard_metadata.ingress_global_timestamp - _startTime>=flow_level_window) {
            ...
            linkLoad.read(link_load, (bit<32>)standard_metadata.ingress_port); //read from previously saved link load
